...
namespace
{
    void PrintUsage(const char* program)
    {
        std::cout << "Usage: " << program << " [options]\n"
                  ...
                  << "  --host <host>                     Redis listen host (default: 127.0.0.1)\n"
                  << "  --port <port>                     Redis listen port (default: 6379)\n"
...
int main(int argc, char* argv[])
{
    ...
    bitcask::redis::ServerOptions server_options;
    ...
    std::cout << "bitcask_redis listening on redis://" << server_options.host << ':' << server_options.port << '\n';
    if (!bitcask::redis::Listen(*db, server_options))
    {
